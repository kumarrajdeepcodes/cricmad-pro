...
// --- NEW ROUTE: GET UNIQUE SERIES NAMES ---
app.get("/api/series", authenticateToken, async (req, res) => {
    try {
        const query = req.user.role === "superadmin" ? {} : { createdBy: req.user.id };
        // Fetch distinct series names from the Match collection
        const series = await Match.distinct("seriesName", query);
        res.json(series.filter(s => s)); // Filter out null/empty values
    } catch (e) {
